    ...
    def __init__(self, field):
        ...
        for x in range(field.size[0]):
            for y in range(1, field.size[1] + 1):
                ...
                if x != 0 and field.field[x - 1][y].patency():
                    self.graf[x][y].add_new_neighbors(self.graf[x - 1][y])
                    if y != 0 and field.field[x - 1][y - 1].patency():
                        self.graf[x][y].add_new_neighbors(self.graf[x - 1][y - 1])
                    if y != field.size[1] - 1 and field.field[x - 1][y + 1].patency():
                        self.graf[x][y].add_new_neighbors(self.graf[x - 1][y + 1])

                if x != field.size[0] - 1 and field.field[x + 1][y].patency():
                    self.graf[x][y].add_new_neighbors(self.graf[x + 1][y])
                    if y != 0 and field.field[x + 1][y - 1].patency():
                        self.graf[x][y].add_new_neighbors(self.graf[x + 1][y - 1])
                    if y != field.size[1] - 1 and field.field[x + 1][y + 1].patency():
